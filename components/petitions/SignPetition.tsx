...
export default function SignPetition() {
  ...
  const { data: petition } = useSWR(
    router.query.id && ['/api/v1/petitions', { id: router.query.id }],
  );
  const { data: signature } = useSWR(
    router.query.id && ['/api/ajax/signatures', { petition: router.query.id }],
  );
  ...
  return (
    <>
      <button
        ...
        disabled={!!signature}
        className={clsx(
          'inline-flex justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:bg-gray-100 focus:outline-none',
          {
                      ...
                      <div className="sm:flex sm:items-start">
                        <div className="mt-3 text-center sm:mt-0 sm:text-left">
                          ...
                          <Dialog.Description className="mt-2 text-sm text-gray-500">
                            Are you sure you want to sign the "{petition?.title}
